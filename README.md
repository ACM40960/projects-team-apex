...
Deep learning classifiers for mammographic malignancy detection output a softmax probability, but that probability is not a trustworthy confidence score on its own... This project's own base classifier has an Expected Calibration Error of ~0.205, meaning "80% confident" is not actually right 80% of the time. This project wraps a DenseNet-121 classifier, fine-tuned on the CBIS-DDSM (benign vs. malignant) dataset, with split conformal prediction: a post-hoc, model-agnostic calibration layer that produces prediction sets with a distribution-free, finite-sample coverage guarantee — valid regardless of whether the underlying model is well-calibrated.
...
See Code/README.md for exact run instructions (Kaggle, GPU T4 ×2, dataset mount path, smoke test vs. full run). 
Outputs land in Outputs Received/; running Code/test_coverage.py and Code/test_notebook_matches_conformal.py verifies the conformal implementation independently of any specific run.
